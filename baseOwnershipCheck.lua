		...
		for _k, base in pairs(world.getAirbases()) do -- WARNING: MOOSE: AIRBASE.GetAllAirbases will not return CONTESTED bases where side == nil
			...
			if _DCSbaseCategory == Airbase.Category.AIRDROME then
				...
				if _logisticsCentre ~= nil and _logisiticsCentreLife > 0 then
					...
					elseif _RSRowner ~= _logisticsCentreSide then --required if neutral airbase has no units on it but player builds logisitics centre
			...
			elseif _DCSbaseCategory == Airbase.Category.HELIPAD then
				...
				if _logisticsCentre ~= nil and _logisiticsCentreLife > 0 then
					...
					if _RSRowner ~= _DCSsideName then
						...
						else --if _DCSsideName ~= "neutral" then --unnecessary
